...
class MultiAgentTrajectoryCollector(TrajectoryCollector):
    ...
    def vanilla_multi_turn_loop(
        ...
        if getattr(self.multiagent_orchestra, "value_metadata_enabled", False):
            self.multiagent_orchestra.prepare_value_metadata(
                gen_batch, gen_batch.non_tensor_batch.get("env_kwargs")
            )
        obs, infos = envs.reset(kwargs=gen_batch.non_tensor_batch.pop('env_kwargs', None))
        ...
        for _step in range(self.config.env.max_steps):
            ...
            for data in multiagent_batch_buffer:
                ...
                for key in ("curriculum_dataset_index", "curriculum_source"):
                    if key in gen_batch.non_tensor_batch:
                        agent_batch.non_tensor_batch[key] = gen_batch.non_tensor_batch[key].copy()
