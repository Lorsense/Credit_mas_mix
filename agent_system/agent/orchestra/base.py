...
class BaseOrchestra:
    ...
    def __init__(
        ...
        # Per-role, per-sample counters.  They live for the whole trajectory
        # collection and are deliberately not reset by reset_buffer(), which is
        # called once per outer environment step in SearchQA.
        self._role_turn_counts: Dict[str, np.ndarray] = {}
        self.memory = None

    def reset(self):
        """Reset the orchestra, all agents, and buffer."""
        self.reset_buffer()
        self._role_turn_counts.clear()
        self.memory = None
    ...
    def save_to_buffer(self, name: str, batch: DataProto):
        ...
        """
        if "agent_active_mask" not in batch.non_tensor_batch:
            raise KeyError("agent_active_mask is required to assign role_turn_index")
        active_mask = np.asarray(batch.non_tensor_batch["agent_active_mask"], dtype=bool)
        batch_size = len(batch)
        if active_mask.shape != (batch_size,):
            raise ValueError(
                f"agent_active_mask for {name!r} must have shape ({batch_size},), "
                f"got {active_mask.shape}"
            )

        if name not in self._role_turn_counts:
            self._role_turn_counts[name] = np.zeros(batch_size, dtype=np.int64)
        counts = self._role_turn_counts[name]
        if counts.shape != (batch_size,):
            raise ValueError(
                f"batch size for {name!r} changed within one trajectory collection: "
                f"{counts.shape[0]} -> {batch_size}"
            )

        # Zero-based and independent for every role and trajectory sample.
        # Inactive filler rows are marked -1 and are discarded by the collector.
        role_turn_index = np.full(batch_size, -1, dtype=np.int64)
        role_turn_index[active_mask] = counts[active_mask]
        batch.non_tensor_batch["role_turn_index"] = role_turn_index
