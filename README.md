...
本目录的 mix 实现包含 pure 两阶段信用分配、baseline + sup 离线熵感知价值预训练，以及条件 Actor 熵约束。16 卡启动、数据格式和恢复说明见 [mix 训练文档](docs/entropy_value_mix.md)。`examples/drmas_trainer/run_math.sh` 默认使用 15 张共享 Agent GPU + 1 张价值 GPU。

新增双向零优势恢复与题目缓存重采样，默认 16 卡入口已启用。流程、参数、断点恢复和消融见 [零优势恢复说明](docs/advantage_recovery_mix.md)。

价值初始化现支持直接读取 baseline 的逐 global-step JSON 文件；两轮历史先验证绝对熵，三轮主训练再逐角色启用合格时序分支。可配置 Bash 和数据字段说明见 [baseline 价值预训练](docs/baseline_value_pretrain.md)。
