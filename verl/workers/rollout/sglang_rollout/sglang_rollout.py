...
def _post_process_outputs(tokenizer, output):
    def _map_each_response(resp):
        ...
        top16_entropy = resp["meta_info"].get("top16_entropy")
        return torch.tensor(output_token_ids), torch.tensor(log_probs), top16_entropy

    out_map = map(lambda x: _map_each_response(x), output)
    batched_output_token_ids = []
    batched_logprobs = []
    top16_entropy_stats = []
    for output_token_ids, log_probs, top16_entropy in out_map:
        batched_output_token_ids.append(output_token_ids)
        batched_logprobs.append(log_probs)
        top16_entropy_stats.append(top16_entropy)
    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id
    batched_output_token_ids = pad_sequence(batched_output_token_ids, batch_first=True, padding_value=pad_token_id)
    if len(batched_logprobs) > 0:
        batched_logprobs = pad_sequence(batched_logprobs, batch_first=True, padding_value=pad_token_id)
    return batched_output_token_ids, batched_logprobs, top16_entropy_stats


def _attach_action_topk_entropy(output, top_k: int) -> None:
    """Aggregate Top-K distributions on TP rank 0, then discard the raw lists."""
    ...
    for response in responses:
        ...
        # Only compact action-level statistics are broadcast and retained.
    ...
    def _init_sampling_params(self, **kwargs):
        ...
        for k in self.config.keys():
            if k == "top_logprobs_num":
                # This belongs to Engine.async_generate, not SamplingParams.
                continue
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            if self._tp_rank == 0:
                loop = asyncio.get_event_loop()
                engine_kwargs = {
                    "prompt": None,  # already converted to prompt token ids
                ...
                if self.top_logprobs_num > 0:
                    # SGLang exposes Top-K logprobs as a request-level argument,
                    # not as a SamplingParams field.
            ...
            if self.top_logprobs_num > 0:
                ...
                _non_tensor_batch["top16_entropy_mean"] = np.asarray(
                    [
                        np.nan if stats is None else float(stats["mean"])
                        for stats in top16_entropy_stats
                    ],
                    dtype=np.float64,
                )
