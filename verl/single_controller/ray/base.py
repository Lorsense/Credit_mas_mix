...
class RayResourcePool(ResourcePool):
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None, device_name="cuda"):
        ...
        if len(set(self._store)) > 1 and strategy == "STRICT_PACK":
            # With 7+8 available GPUs, placing the seven-GPU group on the
            # eight-GPU host first strands the larger group indefinitely.
            # Reserve largest bundles first and wait before allocating smaller
            # ones. Return in logical order; ranks are still sorted by node IP.
            indexed_pgs = {}
            for idx in sorted(range(len(pg_scheme)), key=lambda i: len(pg_scheme[i]), reverse=True):
                pg = placement_group(bundles=pg_scheme[idx], strategy=strategy,
                                     name=pg_name_prefix + str(idx), lifetime=lifetime)
                ray.get(pg.ready())
                indexed_pgs[idx] = pg
            pgs = [indexed_pgs[idx] for idx in range(len(pg_scheme))]
        else:
...
class RayWorkerGroup(WorkerGroup):
    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached):
        ...
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            # A dedicated value GPU can leave heterogeneous Agent node sizes
            # (e.g. 7+8 on two eight-GPU hosts). IP sorting may reorder bundles.
